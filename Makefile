...
deps: bin/golangci-lint bin/trivy bin/regcopy bin/jq bin/yq bin/crane werf ## Install dev dependencies.
...
bin/werf: bin bin/trdl ## Install werf for images-tags generator.
	...
	ln -sf $$(trdl bin-path werf 1.2 stable)/werf $(PWD)/bin/werf

.PHONY: werf
werf: ## Install werf for images-tags generator.
	command -V werf || make bin/werf
